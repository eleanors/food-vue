...
export function _stringify(val) {
        let returnVal = is.Object(val) ? JSON.stringify(val) : val;
        return returnVal;
}
        ...
        return (function format(timestamp, strType = 1, serverTime = 0, noFixTimezone = false) {
                ...
                switch (strType) {
                        ...
                        case 4:
                                ...
                                formatDate = year + '-' + fillZero(month) + '-' + fillZero(date);
                                break;
...
export function extend(src, des, d) {
        let depth = (d) ? d : 0;
        for (let key in src) {
                let isObject = is.Object(src[key]);
                let isArray = is.Array(src[key]);
                if (isObject || isArray) {
                        if (depth) {
                                if (isObject) {
                                        des[key] = {};
